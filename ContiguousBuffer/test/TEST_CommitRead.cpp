...
TEST_F(TEST_CommitRead, CommitReadOperations_ReadAt0) {
    ...
    EXPECT_EQ(mRingBuffer.Size(), 0);
    EXPECT_TRUE(mRingBuffer.CommitRead(0));
    EXPECT_EQ(mRingBuffer.Size(), 0);

    mRingBuffer.SetState(0, 0, 4);
    EXPECT_FALSE(mRingBuffer.CommitRead(1));
    EXPECT_EQ(mRingBuffer.Size(), 0);
    ...
    EXPECT_TRUE(mRingBuffer.CommitRead(1));
    EXPECT_EQ(mRingBuffer.Size(), 0);

    mRingBuffer.SetState(1, 0, 4);
    EXPECT_FALSE(mRingBuffer.CommitRead(2));
    EXPECT_EQ(mRingBuffer.Size(), 1);
    ...
    mRingBuffer.SetState(2, 0, 4);
    EXPECT_TRUE(mRingBuffer.CommitRead(2));
    EXPECT_EQ(mRingBuffer.Size(), 0);

    mRingBuffer.SetState(2, 0, 4);
    EXPECT_FALSE(mRingBuffer.CommitRead(3));
    EXPECT_EQ(mRingBuffer.Size(), 2);
    ...
    mRingBuffer.SetState(3, 0, 4);
    EXPECT_FALSE(mRingBuffer.CommitRead(4));
    EXPECT_EQ(mRingBuffer.Size(), 3);
}

TEST_F(TEST_CommitRead, CommitReadOperations_ReadAt1) {
...
TEST_F(TEST_CommitRead, CommitReadOperations_ReadAt2) {
    ...
    EXPECT_TRUE(mRingBuffer.CommitRead(1)); // Uses the available element at end
    EXPECT_EQ(mRingBuffer.Size(), 2);

    mRingBuffer.SetState(1, 2, 4);
    EXPECT_TRUE(mRingBuffer.CommitRead(2)); // Uses the available element at end
    EXPECT_EQ(mRingBuffer.Size(), 1);

    mRingBuffer.SetState(1, 2, 4);
    EXPECT_FALSE(mRingBuffer.CommitRead(3)); // Cannot read over the wrap, must be in 2 iterations
    EXPECT_EQ(mRingBuffer.Size(), 3);

    mRingBuffer.SetState(1, 2, 4);
    EXPECT_TRUE(mRingBuffer.CommitRead(2));
    EXPECT_EQ(mRingBuffer.Size(), 1);
    EXPECT_TRUE(mRingBuffer.CommitRead(1)); // Uses the available element at start
    EXPECT_EQ(mRingBuffer.Size(), 0);
...
TEST_F(TEST_CommitRead, CommitReadOperations_ReadAt3) {
    ...
    mRingBuffer.SetState(1, 3, 4);
    EXPECT_FALSE(mRingBuffer.CommitRead(2)); // Cannot read over the wrap, must be in 2 iterations
    EXPECT_EQ(mRingBuffer.Size(), 2);
    ...
    EXPECT_TRUE(mRingBuffer.CommitRead(1));
    EXPECT_EQ(mRingBuffer.Size(), 2);
    EXPECT_TRUE(mRingBuffer.CommitRead(1)); // Uses the available elements at start
    EXPECT_EQ(mRingBuffer.Size(), 1);
...
TEST_F(TEST_CommitRead, CommitReadOperations_InvalidStates) {
    ...
    EXPECT_FALSE(mRingBuffer.CommitRead(1)); // Handle as: buffer empty
    EXPECT_EQ(mRingBuffer.Size(), 0);
