...
TEST_F(TEST_CommitWrite, BasicOperations_ReadAt0) {
    ...
    EXPECT_EQ(mRingBuffer.Size(), 0);
    EXPECT_TRUE(mRingBuffer.CommitWrite(0));
    EXPECT_EQ(mRingBuffer.Size(), 0);
    ...
    mRingBuffer.SetState(0, 0, 4);
    EXPECT_FALSE(mRingBuffer.CommitWrite(4));
    EXPECT_EQ(mRingBuffer.Size(), 0);
    ...
    mRingBuffer.SetState(1, 0, 4);
    EXPECT_FALSE(mRingBuffer.CommitWrite(3));
    EXPECT_EQ(mRingBuffer.Size(), 1);
    ...
    mRingBuffer.SetState(2, 0, 4);
    EXPECT_FALSE(mRingBuffer.CommitWrite(2));
    EXPECT_EQ(mRingBuffer.Size(), 2);
    ...
    EXPECT_FALSE(mRingBuffer.CommitWrite(1));
    EXPECT_EQ(mRingBuffer.Size(), 3);
}

TEST_F(TEST_CommitWrite, BasicOperations_ReadAt1) {
...
TEST_F(TEST_CommitWrite, BasicOperations_ReadAt2) {
...
TEST_F(TEST_CommitWrite, BasicOperations_ReadAt3) {
    ...
    EXPECT_TRUE(mRingBuffer.CommitWrite(1)); // Uses the available element at the end
    EXPECT_EQ(mRingBuffer.Size(), 1);
    EXPECT_TRUE(mRingBuffer.CheckState(0, 3, 4)); // Does not wrap

    mRingBuffer.SetState(3, 3, 4);
    EXPECT_TRUE(mRingBuffer.CommitWrite(2)); // Uses the available elements at the start
    EXPECT_EQ(mRingBuffer.Size(), 2);
...
TEST_F(TEST_CommitWrite, BasicOperations_InvalidStates) {
    ...
    EXPECT_FALSE(mRingBuffer.CommitWrite(1)); // Handle as: buffer full
    EXPECT_EQ(mRingBuffer.Size(), 3);
