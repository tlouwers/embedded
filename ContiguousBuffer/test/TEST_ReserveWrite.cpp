...
class TEST_ReserveWrite : public ::testing::Test {
protected:
...
TEST_F(TEST_ReserveWrite, ReserveWriteOperations_ReadAt0) {
...
TEST_F(TEST_ReserveWrite, ReserveWriteOperations_ReadAt1) {
...
TEST_F(TEST_ReserveWrite, ReserveWriteOperations_ReadAt2) {
...
TEST_F(TEST_ReserveWrite, ReserveWriteOperations_ReadAt3) {
    ...
    size = 1;
    EXPECT_TRUE(mRingBuffer.ReserveWrite(data, size)); // Can use the available element at the end
    EXPECT_EQ(size, 1);

    size = 2;
    EXPECT_TRUE(mRingBuffer.ReserveWrite(data, size)); // Can use the available elements at the start
    EXPECT_EQ(size, 2);
...
TEST_F(TEST_ReserveWrite, InvalidStates) {
    ...
    size_t size = 1;
    EXPECT_FALSE(mRingBuffer.ReserveWrite(data, size)); // Handle as: buffer full
    EXPECT_EQ(size, 0);
