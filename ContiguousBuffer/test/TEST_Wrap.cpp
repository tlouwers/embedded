...
TEST_F(TEST_Wrap, SmallestWrapPossible) {
    ...
    EXPECT_TRUE(mRingBuffer.CommitRead(1)); // Read 1 element, restores wrap
    ...
    EXPECT_TRUE(mRingBuffer.CommitRead(2)); // Read 2 elements, restores wrap
    ...
    size = 3;
    EXPECT_TRUE(mRingBuffer.ReserveWrite(data, size)); // Exception: if write == read and size == read allow Poke() to reset mWrite and mRead
    EXPECT_EQ(size, 3);
}
...
TEST_F(TEST_Wrap, WrapWithSmallBufferReadStartsUnequalToWrap) {
    ...
    EXPECT_TRUE(mRingBuffer.CommitWrite(1));
    EXPECT_EQ(mRingBuffer.Size(), 1);

    size = 2;
    EXPECT_TRUE(mRingBuffer.ReserveWrite(data, size)); // Only 1 element available at end, but 2 available at start
    EXPECT_EQ(size, 2);
    ...
    size = 2;
    EXPECT_FALSE(mRingBuffer.ReserveRead(data, size)); // Only 1 element present at end, 2 elements present at start, buffer wrapped
    EXPECT_EQ(size, 0);
    ...
    size = 1;
    EXPECT_TRUE(mRingBuffer.ReserveRead(data, size)); // 2 elements present at start
    EXPECT_EQ(size, 2);
    size = 3;
    EXPECT_FALSE(mRingBuffer.ReserveRead(data, size)); // 2 elements present at start
    EXPECT_EQ(size, 0);
    ...
    EXPECT_EQ(data[0], 1);
    EXPECT_EQ(data[1], 2);
    EXPECT_TRUE(mRingBuffer.CommitRead(2)); // Read 2 elements
...
TEST_F(TEST_Wrap, WrapWithSmallBufferReadStartsEqualToWrap) {
    ...
    size = 1;
    EXPECT_TRUE(mRingBuffer.ReserveRead(data, size)); // 2 elements present at start, buffer wrapped
    EXPECT_EQ(size, 2);
...
TEST_F(TEST_Wrap, WrapWithLargeBuffer) {
    ...
    size_t size = 1;
    EXPECT_TRUE(mRingBuffer.ReserveWrite(data, size)); // 7 contiguous elements available at the end
    EXPECT_EQ(size, 7);

    size = 13;
    EXPECT_TRUE(mRingBuffer.ReserveWrite(data, size)); // And 13 contiguous elements available at the start
    EXPECT_EQ(size, 13);

    EXPECT_TRUE(mRingBuffer.CommitWrite(10)); // Wraps, leave 3 elements available
    ...
    size = 1;
    EXPECT_TRUE(mRingBuffer.ReserveWrite(data, size)); // 3 elements available
    EXPECT_EQ(size, 3);

    EXPECT_TRUE(mRingBuffer.CommitRead(3)); // Read 3 elements, restores wrap, leaving 7 remaining
    ...
    EXPECT_TRUE(mRingBuffer.CommitRead(7)); // Read remaining 7 elements
