...
TEST_CASE( "Ringbuffer TryPush() operations", "[Ringbuffer]" )
{
    ...
    SECTION( "basic operations - read at 0" )
    {
        ...
        REQUIRE(ringBuff.TryPush(pSrc, 0) == false);
        ...
        // Default: size == 1
        ringBuff.SetState(0, 0);
        REQUIRE(ringBuff.TryPush(pSrc) == true);
        REQUIRE(ringBuff.CheckState(1, 0) == true);
        ...
        ringBuff.SetState(0, 0);
        REQUIRE(ringBuff.TryPush(pSrc, 4) == false);
        ...
        ringBuff.SetState(1, 0);
        REQUIRE(ringBuff.TryPush(pSrc, 3) == false);
        ...
        ringBuff.SetState(2, 0);
        REQUIRE(ringBuff.TryPush(pSrc, 2) == false);
        ...
        REQUIRE(ringBuff.TryPush(pSrc, 1) == false);

        // Default: size == 1
        ringBuff.SetState(3, 0);
        REQUIRE(ringBuff.TryPush(pSrc) == false);
    }
    ...
    SECTION( "basic operations - invalid states" )
    {
        ...
        REQUIRE(ringBuff.TryPush(pSrc, 1) == false);    // Handle as: buffer full
